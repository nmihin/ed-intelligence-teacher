    ...
    <div class="container-fluid student-list attendance">
      <div class="row">
        ...
        <div v-if="viewType === 'download'" class="col-12 col-sm-6 col-md-8">
          <button
            v-if="viewType === 'download'"
            class="download-button button medium ed-btn__primary"
            @click="downloadExcelTemplate()"
          >
            <span class="icon icon-download-excel"
        ...
        <div class="col-12 col-sm-6 col-md-4">
          <button
            @click="viewTypeSelect('download')"
            class="change-view upload button medium ed-btn__secondary"
          >
            <i class="icon icon-upload"></i>
          </button>
          <button
            @click="viewTypeSelect('avatar')"
            class="change-view button medium ed-btn__primary"
          >
            <i class="icon icon-manage"></i>
          </button>
          <button
            @click="viewTypeSelect('list')"
            class="change-view menu-list button medium ed-btn__primary"
          >
            <i class="icon icon-menu-list"></i>
          </button>
        </div>
        <div
          v-if="viewType === 'list' || viewType === 'avatar'"
      ...
      >
        ...
        <el-table
          ...
          stripe
          ref="singleTable"
          :data="studentAttendances.formData"
        ...
        >
          ...
          <el-table-column sortable property="status" label="Status">
            <div slot-scope="scope">
              <el-form-item>
                <el-radio-group 
                  @change="updateAttendanceList(scope.row.sn,scope.row.status,'status')"
          ...
          <el-table-column sortable property="reason" label="Reason">
            <div slot-scope="scope">
              <el-form-item
                :prop="'formData.' + scope.$index + '.reason'"
                :rules="rules.reason"
                v-if="scope.row.status === 'Present'"
              >
                <el-select 
                  @change="updateAttendanceList(scope.row.sn,scope.row.reason,'present')"
                  v-model="scope.row.reason" placeholder="Present">
                  <el-option
                    v-for="pre in reasonPresentOptionsSelect"
                    :key="pre.value"
                    :label="pre.label"
                    :value="pre.value"
                  >
              ...
              <el-form-item
                :prop="'formData.' + scope.$index + '.reason'"
                :rules="rules.reason"
                v-if="scope.row.status === 'Absent'"
              >
                <el-select 
                  @change="updateAttendanceList(scope.row.sn,scope.row.reason,'absent')"
                  v-model="scope.row.reason" placeholder="Absent">
                  <el-option
                    v-for="pre in reasonAbsentOptionsSelect"
                    :key="pre.value"
                    :label="pre.label"
                    :value="pre.value"
                  >
              ...
              <el-form-item
                :prop="'formData.' + scope.$index + '.absentReason'"
                :rules="rules.absentReason"
                v-if="scope.row.status === 'Absent'"
              >
                <el-select
                  @change="updateAttendanceList(scope.row.sn,scope.row.absentReason,'absentReason')"
                  v-model="scope.row.absentReason"
                  placeholder="Absent Reason"
                >
                  <el-option
                    v-for="pre in reasonAbsentReasonOptionsSelect"
                    :key="pre.value"
                    :label="pre.label"
                    :value="pre.value"
                  >
        ...
        <div v-if="viewType === 'avatar'" class="row search-results">
          <div
            v-for="(item, index) in studentAttendances.formData"
            :key="index"
            class="col-12 col-md-6 col-lg-6 col-xl-4 ed_card-boxes"
          >
            <div class="card-box">
              ...
              <div class="card-content">
                <figure>
                  <img
                    v-if="!item.avatar"
                    class="card-picture"
                    src="../assets/images/avatar-aux.png"
                  />
                  <img
                    v-if="item.avatar"
                    class="card-picture"
                  ...
                  <figcaption>
                    <ul>
                      <li class="attendance-list-avatar">
                        <h3>Status</h3>
                        <el-form-item
                          :prop="'formData.' + index + '.status'"
                          :rules="rules.status"
                        >
                          <el-radio-group 
                            @change="updateAttendanceList(item.sn,item.status,'status')"
                            v-model="item.status">
                            <el-radio label="Present">Present</el-radio>
                            <el-radio label="Absent">Absent</el-radio>
                          </el-radio-group>
                      ...
                      <li class="attendance-list-avatar">
                        ...
                        >
                          <el-select 
                            @change="updateAttendanceList(item.sn,item.reason,'present')"          
                            v-model="item.reason" placeholder="Reason">
                            <el-option
                              v-for="pre in reasonPresentOptionsSelect"
                              :key="pre.value"
                              :label="pre.label"
                        ...
                        >
                          <el-select 
                            @change="updateAttendanceList(item.sn,item.reason,'absent')"
                            v-model="item.reason" placeholder="Reason">
                            <el-option
                              v-for="pre in reasonAbsentOptionsSelect"
                              :key="pre.value"
                        ...
                        <el-form-item
                          v-if="item.status === 'Absent'"
                          :prop="'formData.' + index + '.absentReason'"
                          :rules="rules.absentReason"
                        >
                          <el-select
                            @change="updateAttendanceList(item.sn,item.reason,'absentReason')"
                            v-model="item.absentReason"
                            placeholder="Reason"
                          >
                            <el-option
                              v-for="pre in reasonAbsentReasonOptionsSelect"
              ...
              <div class="card-content">
                ...
                >
                  ...
                  <el-button
                    ...
                    @click="submitUploadedDocument()"
                    ><i class="icon icon-upload"></i>Upload to server</el-button
                  >
      ...
      <div class="attendance-bottom">
        <button
          v-if="viewType === 'list' || viewType === 'avatar'"
          class="attendence-save-changes button medium ed-btn__primary"
          @click="validatestudentAttendances()"
        >
          Save Changes
        </button>
        <el-pagination
  ...
  data() {
    return {
      ...
      reasonAbsentReasonOptionsSelect: [
        ...
        {
          value:
            "[17] Temporary relocation due to closing of facilities or suspension of classes",
          label:
            "[17] Temporary relocation due to closing of facilities or suspension of classes",
        },
      ],
